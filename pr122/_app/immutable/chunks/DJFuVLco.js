import{j as g,s as w}from"./Ch7xDvpF.js";import{t as h}from"./BXWuFe6I.js";function F(i,o){if(i.length===0)return[];const e=2/(o+1),r=[];let t=i[0];r.push(t);for(let n=1;n<i.length;n++)t=i[n]*e+t*(1-e),r.push(t);return r}function p(i,o){const e=[],r=new Date(i);for(let t=o-1;t>=0;t--){const n=new Date(r);n.setDate(n.getDate()-t),e.push(n.toISOString().split("T")[0])}return e}function k(i,o,e,r=30,t=14){const c=p(e,r).map(u=>i[u]?.[o]||0);return F(c,t)}let f=null;async function A(i){const e=await fetch("https://generativelanguage.googleapis.com/v1beta/models",{headers:{Authorization:`Bearer ${i}`}});if(!e.ok)throw new Error(`Failed to fetch Gemini models: ${e.status}`);return(await e.json()).models||[]}async function y(i){const o=w.getState().config.geminiModel;if(o)return f!==o&&(h.info(`Using AI model: ${o}`),f=o),o;if(f)return f;try{const r=(await A(i)).filter(t=>t.name&&t.name.includes("flash")&&t.supportedGenerationMethods&&t.supportedGenerationMethods.includes("generateContent")).map(t=>t.name.replace("models/","")).sort((t,n)=>t.endsWith("-latest")&&!n.endsWith("-latest")?-1:!t.endsWith("-latest")&&n.endsWith("-latest")?1:n.localeCompare(t));if(r.length>0){const t=r[0];return f!==t&&(h.info(`Using AI model: ${t}`),f=t),t}}catch(e){throw console.warn("Error fetching Gemini models",e),e}throw new Error("No valid Gemini Flash models found")}const E=`
...
`;async function C(i,o,e){const r=await g();if(!r)throw new Error("User not authenticated for Gemini analysis");const n=`https://generativelanguage.googleapis.com/v1beta/models/${await y(r)}:generateContent`;let c=E;i.text&&(c+=`

USER TEXT DESCRIPTION: "${i.text}"
`),o&&e&&(c+=`
        ...
        `);const u=[{text:c}];i.images&&i.images.forEach(d=>{u.push({inlineData:{mimeType:d.mimeType,data:d.base64}})});const s=await fetch(n,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${r}`},body:JSON.stringify({contents:[{parts:u}],generationConfig:{responseMimeType:"application/json"}})});if(!s.ok){const d=await s.text();throw new Error(`Gemini API Error: ${s.status} - ${d}`)}const b=(await s.json()).candidates?.[0]?.content?.parts?.[0]?.text;if(!b)throw new Error("No content in Gemini response");return JSON.parse(b)}function $(i,o,e,r){const t=i,c=p(t,14),u=o.filter(a=>c.includes(a.date));let s=`Target Calories: ${e.targetCalories} kcal
...
${JSON.stringify(i.map(a=>{const l={};return o.fiberGoal.enabled&&a.details?.fiber!==void 0&&(l.fiber=a.details.fiber),o.sodiumGoal.enabled&&a.details?.sodium!==void 0&&(l.sodium=a.details.sodium),o.sugarLimit.enabled&&a.details?.sugar!==void 0&&(l.sugar=a.details.sugar),o.addedSugarLimit.enabled&&a.details?.addedSugar!==void 0&&(l.addedSugar=a.details.addedSugar),o.satFatLimit.enabled&&a.details?.saturatedFat!==void 0&&(l.saturatedFat=a.details.saturatedFat),o.transFatLimit.enabled&&a.details?.transFat!==void 0&&(l.transFat=a.details.transFat),o.cholesterolLimit.enabled&&a.details?.cholesterol!==void 0&&(l.cholesterol=a.details.cholesterol),{date:a.date,time:a.time,description:a.description,calories:a.calories,protein:a.protein,carbs:a.carbs,fat:a.fat,details:l}}))}
