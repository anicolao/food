import{e as c}from"./DBYtN6AS.js";const f=`
...
`;async function h(n,t,s){const d=await c();if(!d)throw new Error("User not authenticated for Gemini analysis");const l="https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent";let o=f;n.text&&(o+=`
...
`),t&&s&&(o+=`
        ...
        `);const u=[{text:o}];n.images&&n.images.forEach(e=>{u.push({inlineData:{mimeType:e.mimeType,data:e.base64}})});const a=await fetch(l,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${d}`},body:JSON.stringify({contents:[{parts:u}],generationConfig:{responseMimeType:"application/json"}})});if(!a.ok){const e=await a.text();throw new Error(`Gemini API Error: ${a.status} - ${e}`)}const r=(await a.json()).candidates?.[0]?.content?.parts?.[0]?.text;if(!r)throw new Error("No content in Gemini response");return JSON.parse(r)}async function p(n,t,s,d){const l=await c();if(!l)throw new Error("User not authenticated for AI feedback");const o="https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent",u=`
...
`,a=await fetch(o,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${l}`},body:JSON.stringify({contents:[{parts:[{text:u}]}]})});if(!a.ok){const e=await a.text();throw new Error(`Gemini API Error: ${a.status} - ${e}`)}const r=(await a.json()).candidates?.[0]?.content?.parts?.[0]?.text;if(!r)throw new Error("No content in Gemini response");return r}export{h as a,p as g};
